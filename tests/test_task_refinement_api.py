...
@pytest.mark.asyncio
async def test_task_refinement_endpoint(refinement_client, monkeypatch):
    client, SessionLocal = refinement_client

    refined_task_id = {"value": None}
    ...
    async def seed():
        async with SessionLocal() as session:
            ...
            refined_task_id["value"] = task.id
            return sprint.id

    sprint_id = await seed()

    resp = client.post(f"/sprints/{sprint_id}/tasks/refine-pass2")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
    assert data[0]["refinement_round"] == 2
